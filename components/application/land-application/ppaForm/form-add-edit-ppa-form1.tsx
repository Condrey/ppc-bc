...
import { useUpsertPpaForm1Mutation } from "./mutations";
...
export default function FormAddEditLandApplication({
  landApplication,
  open,
  onOpenChange,
}: Props) {
  ...
  const { mutate, isPending } = useUpsertPpaForm1Mutation();
  function handleFormSubmit(input: LandApplicationSchema) {
    mutate(input, {
      onSuccess: () => {
        form.reset();
        onOpenChange(false);
